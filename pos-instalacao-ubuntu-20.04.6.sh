...
#=============================================================================
# Script: pos-instalacao-ubuntu-20.04.6.sh
# Descrição: Script completo de pós-instalação para Ubuntu 20.04.6 LTS
#            Configuração automatizada com aplicativos essenciais
# Autor: Andre Berger
# Data: 13/04/2026
# Versão: 1.0
# Licença: MIT
# Compatibilidade: Ubuntu 20.04.6 LTS (Focal Fossa)
#=============================================================================
...
#    chmod +x pos-instalacao-ubuntu-20.04.6.sh
#
...
#
# NOTA: Este script foi testado no Ubuntu 20.04.6 LTS (Focal Fossa)
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║              🐧 PÓS-INSTALAÇÃO UBUNTU 20.04.6 LTS (FOCAL FOSSA)             ║
║                                                                              ║
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        ...
        print_message "$CYAN" "💡 Execute: sudo ./pos-instalacao-ubuntu-20.04.6.sh"
        exit 1
    fi
...
check_ubuntu_version() {
    ...
    if [[ "$ubuntu_version" != "20.04" ]]; then
        print_message "$YELLOW" "⚠️  Este script foi otimizado para Ubuntu 20.04.6"
...
update_system() {
    ...
    if [[ "$USE_DIALOG" == "true" ]]; then
        (
            ...
            echo "30" ; echo "XXX" ; echo "🔄 Atualizando lista de pacotes..." ; echo "XXX"
            apt-get update -y < /dev/null >> "$LOG_FILE" 2>&1 || sleep 2 && apt-get update -y < /dev/null >> "$LOG_FILE" 2>&1 || true
            
            echo "50" ; echo "XXX" ; echo "⬆️  Instalando atualizações (upgrade)..." ; echo "XXX"
            ...
            echo "80" ; echo "XXX" ; echo "⬆️  Instalando atualizações (dist-upgrade)..." ; echo "XXX"
        ...
        print_message "$BLUE" "🔄 Atualizando lista de pacotes... [30%]"
        ...
        print_message "$BLUE" "⬆️  Instalando atualizações (upgrade)... [50%]"
        DEBIAN_FRONTEND=noninteractive apt-get upgrade -y \
            -o Dpkg::Options::="--force-confdef" \
            -o Dpkg::Options::="--force-confold" \
            -o Dpkg::Options::="--force-confnew" \
        ...
        print_message "$BLUE" "⬆️  Instalando atualizações (dist-upgrade)... [80%]"
        DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade -y \
            -o Dpkg::Options::="--force-confdef" \
            -o Dpkg::Options::="--force-confold" \
            -o Dpkg::Options::="--force-confnew" \
...
configure_portuguese() {
    ...
    print_message "$BLUE" "🌍 Instalando pacotes base... [50%]"
    apt install -y language-pack-pt-base language-pack-gnome-pt-base >> "$LOG_FILE" 2>&1
    print_message "$BLUE" "🌍 Instalando dicionários... [75%]"
...
install_java() {
    print_section "☕ SEÇÃO 9/15: INSTALANDO JAVA"
    update_progress "Java OpenJDK"
    
    print_message "$BLUE" "☕ Instalando OpenJDK (Java)..."
    
    # Ubuntu 20.04 usa OpenJDK 11 e 8 por padrão
    apt install -y default-jdk >> "$LOG_FILE" 2>&1
    apt install -y openjdk-11-jdk >> "$LOG_FILE" 2>&1
    apt install -y openjdk-8-jdk >> "$LOG_FILE" 2>&1 || true
...
install_google_chrome() {
    ...
    cd "$TEMP_DIR"
    wget -q --show-progress https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb -O google-chrome.deb 2>&1 | 
        while IFS= read -r line; do
            if [[ $line =~ ([0-9]+)% ]]; then
                printf "\r${CYAN}📥 Baixando... ${WHITE}${BASH_REMATCH[1]}%%${NC}"
            fi
        done
    echo
    
    print_message "$BLUE" "📦 Instalando Google Chrome... [100%]"
...
install_vlc() {
    ...
    apt install -y browser-plugin-vlc >> "$LOG_FILE" 2>&1 || true
...
install_stacer() {
    ...
    print_message "$BLUE" "🔧 Instalando Stacer via Flatpak..."
    flatpak install -y flathub com.oguzhaninan.Stacer >> "$LOG_FILE" 2>&1
...
install_teamviewer() {
    ...
    cd "$TEMP_DIR"
    if wget -q --timeout=30 https://download.teamviewer.com/download/linux/teamviewer_amd64.deb -O teamviewer.deb >> "$LOG_FILE" 2>&1; then
        apt install -y ./teamviewer.deb >> "$LOG_FILE" 2>&1 || print_message "$YELLOW" "⚠️  Erro ao instalar TeamViewer"
        print_message "$GREEN" "✅ TeamViewer instalado!"
    else
        print_message "$YELLOW" "⚠️  Não foi possível baixar TeamViewer, pulando..."
    fi
}
...
EOF
    ...
    echo -e "${GREEN}✨ Seu Ubuntu 20.04.6 LTS está pronto para uso!${NC}"
    echo
}
...
main() {
    ...
    print_message "$BLUE" "🚀 Iniciando script de pós-instalação Ubuntu 20.04.6 LTS..."
    echo
